...
> 你只需要了解 `OpenID 配置 URL`，`客户端 ID`,`客户端 Secret` (点击 `编辑` 即可查看)    
...
部分客户端强制要求**签名密钥 (jwks)**，比如 `Cloudflare Zero Turst` 的 IDP 配置   
对签名无区分要求的，可直接使用内置自签证书
<img src="/Authentik/Ak-19.png" alt="配置签名密钥" width="600" height="600" />
